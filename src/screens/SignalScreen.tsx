...
function TermStructureCanvas({ rows, atm_iv }: { rows: any[]; atm_iv: number }) {
  ...
  useEffect(() => {
    ...
    const xOf    = (d: number) => pad.l + ((d - minD) / (maxD - minD || 1)) * cW
...
export default function SignalScreen() {
  ...
  const [termData, setTermData] = useState<{ term_structure: any[]; atm_iv: number; skew: number; iv_rank: number; iv_regime: string } | null>(null)

  const loadTermStructure = useCallback(async () => {
    try {
      const d = await apiFetch('/api/vol-term-structure')
      if (d.term_structure?.length) setTermData(d)
    } catch {}
  }, [])

  useEffect(() => {
    loadTermStructure()
    const off = on('update', () => loadTermStructure())
    return off
  }, [loadTermStructure, on])
  ...
  return (
    <>
      ...
      {/* ── Live Skew Monitor / Vol Term Structure ── */}
      {!isNdx && (
        <div className="panel">
          ...
          {termData ? (
            <>
              <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <div style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 700, fontFamily: 'var(--mono)', color: termData.atm_iv > 0.25 ? 'var(--red)' : termData.atm_iv > 0.15 ? 'var(--yellow)' : 'var(--green)' }}>
                    {termData.atm_iv > 0 ? (termData.atm_iv * 100).toFixed(1) + '%' : '--'}
                  </div>
                </div>
                <div style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', textAlign: 'center' }}>
                  <div style={{ fontSize: 7, textTransform: 'uppercase', letterSpacing: '.6px', color: 'var(--muted2)' }}>25Δ Skew</div>
                  <div style={{ fontSize: 15, fontWeight: 700, fontFamily: 'var(--mono)', color: termData.skew > 2 ? 'var(--red)' : termData.skew < -2 ? 'var(--green)' : 'var(--yellow)' }}>
                    {termData.skew != null ? (termData.skew > 0 ? '+' : '') + Number(termData.skew).toFixed(1) : '--'}
                  </div>
                </div>
                <div style={{ flex: 1, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', textAlign: 'center' }}>
                  <div style={{ fontSize: 7, textTransform: 'uppercase', letterSpacing: '.6px', color: 'var(--muted2)' }}>IV Regime</div>
                  <div style={{ fontSize: 11, fontWeight: 700, fontFamily: 'var(--mono)', color: termData.iv_regime === 'EXTREME' || termData.iv_regime === 'HIGH' ? 'var(--red)' : termData.iv_regime === 'ELEVATED' ? 'var(--yellow)' : 'var(--green)' }}>
                    {termData.iv_regime ?? '--'}
                  </div>
                  {termData.iv_rank > 0 && <div style={{ fontSize: 8, color: 'var(--muted2)' }}>IVR {termData.iv_rank.toFixed(0)}</div>}
                </div>
              </div>
              <TermStructureCanvas rows={termData.term_structure} atm_iv={termData.atm_iv} />
              {(() => {
                const rows = termData.term_structure
            ...
            <div style={{ color: 'var(--muted2)', fontSize: 10, textAlign: 'center', padding: 12 }}>
              Loading vol term structure...
            </div>
          )}
        </div>
      )}
